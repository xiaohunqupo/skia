...
template <typename T, RawPtrTraits Traits = 0> class SK_TRIVIAL_ABI raw_ptr {
public:
    SK_ALWAYS_INLINE constexpr raw_ptr() noexcept = default;

    // Deliberately implicit, because raw_ptr is supposed to resemble raw ptr.
    SK_ALWAYS_INLINE constexpr raw_ptr(std::nullptr_t) noexcept {}

    // Deliberately implicit, because raw_ptr is supposed to resemble raw ptr.
    SK_ALWAYS_INLINE constexpr raw_ptr(T* p) noexcept : wrapped_ptr_(p) {}

    // Deliberately implicit in order to support implicit upcast.
    template <typename U,
              typename Unused = std::enable_if_t<std::is_convertible_v<U*, T*> &&
                                                 !std::is_void_v<typename std::remove_cv<T>::type>>>
    SK_ALWAYS_INLINE constexpr raw_ptr(const raw_ptr<U, Traits>& ptr) noexcept
    ...
    template <typename U,
              typename Unused = std::enable_if_t<std::is_convertible_v<U*, T*> &&
                                                 !std::is_void_v<typename std::remove_cv<T>::type>>>
    SK_ALWAYS_INLINE constexpr raw_ptr(raw_ptr<U, Traits>&& ptr) noexcept
    ...
    SK_ALWAYS_INLINE constexpr raw_ptr& operator=(std::nullptr_t) noexcept {
        wrapped_ptr_ = nullptr;
        return *this;
    }

    SK_ALWAYS_INLINE constexpr raw_ptr& operator=(T* p) noexcept {
        wrapped_ptr_ = p;
        return *this;
    }
    ...
    template <typename U,
              typename Unused = std::enable_if_t<std::is_convertible_v<U*, T*> &&
                                                 !std::is_void_v<typename std::remove_cv<T>::type>>>
    SK_ALWAYS_INLINE constexpr raw_ptr& operator=(const raw_ptr<U, Traits>& ptr) noexcept {
        wrapped_ptr_ = ptr.wrapped_ptr_;
        return *this;
    }
    ...
    template <typename U,
              typename Unused = std::enable_if_t<std::is_convertible_v<U*, T*> &&
                                                 !std::is_void_v<typename std::remove_cv<T>::type>>>
    SK_ALWAYS_INLINE constexpr raw_ptr& operator=(raw_ptr<U, Traits>&& ptr) noexcept {
        wrapped_ptr_ = ptr.wrapped_ptr_;
        ptr.wrapped_ptr_ = nullptr;
        return *this;
    }
    ...
    SK_ALWAYS_INLINE constexpr U& operator*() const {
        return *wrapped_ptr_;
    }
    SK_ALWAYS_INLINE constexpr T* operator->() const { return wrapped_ptr_; }

    // Deliberately implicit, because raw_ptr is supposed to resemble raw ptr.
    SK_ALWAYS_INLINE constexpr operator T*() const { return wrapped_ptr_; }

    template <typename U> SK_ALWAYS_INLINE constexpr explicit operator U*() const {
    ...
    SK_ALWAYS_INLINE constexpr raw_ptr& operator++() {
        wrapped_ptr_++;
        return *this;
    }
    SK_ALWAYS_INLINE constexpr raw_ptr& operator--() {
    ...
    SK_ALWAYS_INLINE constexpr raw_ptr operator++(int /* post_increment */) {
        return ++wrapped_ptr_;
    }
    SK_ALWAYS_INLINE constexpr raw_ptr operator--(int /* post_decrement */) {
        return --wrapped_ptr_;
    }
    template <typename Z, typename = std::enable_if_t<partition_alloc::internal::is_offset_type<Z>>>
    SK_ALWAYS_INLINE constexpr raw_ptr& operator+=(Z delta) {
        wrapped_ptr_ += delta;
        return *this;
    }
    template <typename Z, typename = std::enable_if_t<partition_alloc::internal::is_offset_type<Z>>>
    ...
    SK_ALWAYS_INLINE constexpr U& operator[](Z delta) const {
    ...
    SK_ALWAYS_INLINE constexpr void ClearAndDelete() noexcept { delete ExtractAsDangling(); }
    SK_ALWAYS_INLINE constexpr void ClearAndDeleteArray() noexcept { delete[] ExtractAsDangling(); }
    ...
    SK_ALWAYS_INLINE constexpr raw_ptr<T, Traits> ExtractAsDangling() noexcept {
        T* ptr = wrapped_ptr_;
        wrapped_ptr_ = nullptr;
        return raw_ptr(ptr);
    }
    ...
    template <typename U> SK_ALWAYS_INLINE friend bool operator==(const raw_ptr& lhs, U* rhs) {
        return lhs.wrapped_ptr_ == rhs;
    }
    template <typename U> SK_ALWAYS_INLINE friend bool operator!=(const raw_ptr& lhs, U* rhs) {
        return !(lhs == rhs);
    }
    template <typename U> SK_ALWAYS_INLINE friend bool operator==(U* lhs, const raw_ptr& rhs) {
    ...
    SK_ALWAYS_INLINE friend bool operator==(const raw_ptr& lhs, std::nullptr_t) { return !lhs; }
    SK_ALWAYS_INLINE friend bool operator!=(const raw_ptr& lhs, std::nullptr_t) {
        return !!lhs;  // Use !! otherwise the costly implicit cast will be used.
    }
    SK_ALWAYS_INLINE friend bool operator==(std::nullptr_t, const raw_ptr& rhs) { return !rhs; }
    SK_ALWAYS_INLINE friend bool operator!=(std::nullptr_t, const raw_ptr& rhs) {
        return !!rhs;  // Use !! otherwise the costly implicit cast will be used.
    }

    SK_ALWAYS_INLINE friend constexpr void swap(raw_ptr& lhs, raw_ptr& rhs) noexcept {
    ...
    SK_ALWAYS_INLINE constexpr T* get() const { return wrapped_ptr_; }
...
SK_ALWAYS_INLINE bool operator==(const raw_ptr<U, Traits1>& lhs, const raw_ptr<V, Traits2>& rhs) {
